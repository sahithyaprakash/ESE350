...
int main(void)
{
...
#ifdef MASTER
    USART USART_CHANNEL;
    //USART_CHANNEL.USART_BAUDRATE = 57600;
    //USART_CHANNEL.USART_BAUDRATE = 28800;
    ...
    /* Comment this block out to remove the carrier wave.
    clear(TCCR1B, CS12);
    clear(TCCR1B, CS11);
...
//    COIL1_MINUS_ON;
    set(DDRD, 4);
    set(PORTD, 4);  // COIL3_MINUS on

    set(DDRD, 1);
    set(PORTD, 0);  // USART TX is low
    ...
    for(;;){
		...
		m_yellow(TOGGLE);
        set(PORTB, 4);
        USART_Transmit_string("UUUUUU     ");
        //USART_Transmit_string(" Oh, don't speak to me of Austria. Perhaps I don't understand things, but Austria never has wished, and does not wish, for war.   ");
