...
/* addMorseSymbol()
 * adds the specified character to the array of morse
 * symbols
 */
...
/* startNewWord()
 * starts a new word by printing the last letter and adding a space
 */
void startNewWord() {
    startNewLetter();
    putchar(' ');
...
int main (void) {
    ...
    while (1) {
        ...
        TIFR1 |= 0x20; 				//clear input capture flag
        handleSpacing();            //we have an amount of space, handle what happens
        overflows = 0;              //reset overflow cout
        ...
        //determine and store the correct morse character;
